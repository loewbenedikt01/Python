"""
Random Forest — cross-sectional return forecast -> portfolio weights.
...
def rf_targets(db: pd.DataFrame, prices: pd.DataFrame, frequency: str):
    """
    One RF target-weight row per rebalance date for `frequency`, plus a dict of
    ...
    print(f"[rf] {frequency}: first solved {first_solved.date() if first_solved else None}  "
          f"solved {len(rows)}/{len(reb_dates)}")
    if reject:
        r = pd.Series(reject).sort_index()
        early = r[r.index < (first_solved or r.index.max())]
        if len(early):
            print(f"[rf] {frequency}: rejected {early.index.min().date()}..{early.index.max().date()} "
    ...
    if looked:
        print(f"[rf] {frequency}: feature panel cache {cs['hits']}/{looked} hits "
    ...
    hp_spec = pd.DataFrame(GRID).assign(**RF_FIXED,
...
def main() -> None:
    ...
    print(f"[rf] ensemble (no selection): {len(GRID)} configs x {len(BASE_SEED)} seeds "
